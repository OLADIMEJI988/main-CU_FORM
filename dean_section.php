...
$insertSql = "INSERT INTO dean_pending_students (id, stud_name, matric_num, arrived_at) 
                SELECT id, stud_name, matric_num, endorsed_at 
                FROM sub_dean_attended_students 
                WHERE sub_dean_comment IS NOT NULL 
                AND (stud_name, matric_num) NOT IN 
                (SELECT stud_name, matric_num FROM dean_pending_students)
                AND (stud_name, matric_num) NOT IN 
                (SELECT stud_name, matric_num FROM dean_attended_students)
              ";
...
$removedStudents = [];
$removedSql = "SELECT stud_name, matric_num FROM dean_attended_students";
...
<body class="preview-page">
  ...
  <script>
        ...
        // Fetching dean_pending_students if it exists
        $.ajax({
            url: 'fetch_dean_pending_students.php',
            method: 'GET',
            success: function(deanPendingStudents) {
            if (deanPendingStudents && deanPendingStudents.length > 0) {
                students = deanPendingStudents; 
            } else {
                students = studentJs;
            }
            renderStudents();
        ...
        function renderStudents() {
            ...
            if (removedStudent) {
                ...
                success: function() {
                    // Removing the student from the dean pending students list after endorsement/rejection
                    $.ajax({
